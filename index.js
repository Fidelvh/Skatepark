...
const { consultarUsuarios, nuevoUsuario, setUsuarioStatus, conseguirUsuario, setDatosUsuario, eliminarCuenta } = require('./consultas');
console.clear();

//Servidor
app.listen(3000,()=>{console.log(`Server en puerto 3000 con PID: ${process.pid}`)});
...
app.post('/usuario', async (req,res) => {
    const { email,nombre,password,anos,especialidad,nombre_foto } = req.body; 

    try {
        const respuesta = await nuevoUsuario(email,nombre,password,anos,especialidad,nombre_foto);
...
app.post('/registrar',async (req,res) => {
    ...
    const { email,nombre,password,password_2,anos,especialidad } = req.body;
    ...
    if ( password !== password_2) { 
        ...
        try {
            const respuesta = await nuevoUsuario(email,nombre,password,anos,especialidad,name)
            .then(() => {
...
app.put('/datos_perfil', async (req,res) => {
    ...
    try {
        const usuario = await setDatosUsuario(email,nombre,password,anos,especialidad);
        res.status(200).send(usuario);
    } catch (e) {
        res.status(500).send({
            error: `Algo salió mal... ${e}`,
